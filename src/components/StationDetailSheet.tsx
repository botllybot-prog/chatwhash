...
import { MapPin, Clock, MessageCircle, Navigation, Wrench, CalendarCheck, Car } from "lucide-react";
...
const texts = {
  ar: { open: "??????", closed: "?????", slots: "????? ?????", instant: "??? ????", daily: "??? ????", services: "???????", noServices: "?? ???? ?????", minutes: "?????", availableToday: "???????? ??????? ?????", loading: "???? ???????...", noSlots: "?? ???? ?????? ????? ?????", booking: "?????", instantText: "??? ?????? ???? ????? ?????? � ???? ????? ??? ?????? ????? ????", dailyText: "??? ?????? ???? ????? ?????? � ???? ????? ??? ?????? ??????? ?????", chat: "محادثة" },
  en: { open: "Open", closed: "Closed", slots: "Fixed slots", instant: "Instant booking", daily: "Daily booking", services: "Services", noServices: "No services available", minutes: "min", availableToday: "Available time slots today", loading: "Loading...", noSlots: "No slots available today", booking: "Booking", instantText: "This station accepts instant booking � send a WhatsApp message to book now", dailyText: "This station accepts daily booking � send a WhatsApp message to choose the day", chat: "Chat" },
  ku: { open: "?????", closed: "???????", slots: "???? ?????????????", instant: "???? ????", daily: "???? ??????", services: "???????????????", noServices: "??? ?????????????? ????", minutes: "?????", availableToday: "?????? ???????????? ?????", loading: "??????? ?? ???????...", noSlots: "??? ???????? ??????? ????", booking: "???", instantText: "??? ???????? ???? ???? ????????? � ?? ??? ????? ?? ?????? ????? ?????", dailyText: "??? ???????? ???? ?????? ????????? � ?? ?????????? ??? ?? ?????? ????? ?????", chat: "گفتوگۆ" },
  tr: { open: "A�ik", closed: "Kapali", slots: "Sabit saatler", instant: "Anlik rezervasyon", daily: "G�nl�k rezervasyon", services: "Hizmetler", noServices: "Hizmet yok", minutes: "dk", availableToday: "Bug�n uygun saatler", loading: "Y�kleniyor...", noSlots: "Bug�n uygun saat yok", booking: "Rezervasyon", instantText: "Bu istasyon anlik rezervasyon kabul eder � hemen rezervasyon i�in WhatsApp mesaji g�nderin", dailyText: "Bu istasyon g�nl�k rezervasyon kabul eder � g�n� se�mek i�in WhatsApp mesaji g�nderin", chat: "Sohbet" },
} as const;
      ...
      <SheetContent side="bottom" className="h-[85vh] rounded-t-3xl p-0" dir={isRtl ? "rtl" : "ltr"}>
        <ScrollArea className="h-full">
          <div className="p-5 space-y-4">
            ...
            <Button className="w-full gap-2" onClick={() => navigate(`/chat?station_id=${station.id}`)}>
